...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        printf("%ld: %f, %f avg, %f rate, %lf seconds, %d images INGRID\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), what_time_is_it_now()-time, i*imgs);
        
        // Save stats to graph with Excel average progress every 10 batches
        if(i%10==0){
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            // Save stats to graph with Excel average progress
            char* statName = "/stats.txt";
            char* statFile;
            statFile = malloc(strlen(statName) + strlen(backup_directory) + 1);
            strcpy(statFile, backup_directory);
            strcat(statFile, statName);
            FILE *fp = fopen(statFile, "a");
    		//if(!fp) file_error(statFile);
    		fprintf(fp, "%ld, %f, %f\n", get_current_batch(net), loss, avg_loss);
		    fclose(fp);

        }
        // Save checkpoints every indicated batch
        if((i <= 3000 && i%500 == 0) || 
           (i < 10000 && i > 3000 && i%1000 == 0) || 
           (i >= 10000 && i%2000 == 0)){
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
            // Save checkpoint name with average loss
            sprintf(buff, "%s/%s_%d_loss_%f.weights", backup_directory, base, i, avg_loss);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        time=what_time_is_it_now();
        network_predict(net, X);
        printf("%s: Predicted in %f seconds.75435\n", input, what_time_is_it_now()-time);
        int nboxes = 0;
